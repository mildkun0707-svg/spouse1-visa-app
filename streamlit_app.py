...
def send_to_sensei(json_text, client_name):
    ...
    user = _secret(["smtp", "user"])
    password = _secret(["smtp", "password"])
    port = int(_secret(["smtp", "port"], 465) or 465)
    to_addr = _secret(["smtp", "to_email"], user)
    ...
    if not st.session_state["is_admin"]:
        with st.expander("🔑 " + get_text("管理者ログイン", selected_lang)):
            ...
            if st.button(get_text("ログイン", selected_lang), key="_admin_login", use_container_width=True):
                _correct = str(_secret(["admin_password"], "") or "")
                if not _correct:
                    ...
                    if _local.get("__error__"):
                        ...
                        _msg_lines.append("📄 secrets.toml が見つかりませんでした。")
                    st.error("\n\n".join(_msg_lines))
                elif _pw == _correct:
            ...
            # ── 緊急用：secretsを使わず1クリックで管理者へ ──
            # secrets.toml がどうしても読めない環境（Windowsの文字コードトラブル等）でも、
            # 自分のPCで動かしているとき限定で管理者モードに入れるための「裏口」。
            # ※ Streamlit Cloud で公開する場合は、この機能を表示しないように
            #   secrets.toml に `disable_emergency_login = true` を入れてください。
            _disable = bool(_secret(["disable_emergency_login"], False))
            if not _disable:
                st.caption("---")
                st.caption("🆘 secrets.toml が読めないときの応急処置")
                if st.button("🔓 管理者モードに直接入る（自分のPC専用）", key="_admin_force", use_container_width=True):
                    st.session_state["is_admin"] = True
                    st.rerun()
    else:
...
with st.sidebar.expander("🔧 デバッグ情報"):
    ...
    st.write("**🔐 secrets 診断:**")
    _diag = {
        "st.secrets admin_password": "✅ 読めています" if (lambda: True if (lambda: st.secrets.get('admin_password'))() else False)() else "❌ 読めていません",
    }
    try:
        _has_st_secret = bool(st.secrets.get("admin_password"))
    except Exception:
        _has_st_secret = False
    _local = _load_local_secrets()
    st.write({
        "st.secrets で admin_password が読める": _has_st_secret,
        ...
        "最終的に有効な admin_password": "あり" if _secret(["admin_password"], "") else "なし",
    })
    st.write("**ケースフラグ:**")
    st.json(case_flags)
