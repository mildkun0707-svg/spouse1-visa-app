...
def normalize_borders(wb):
    ...
    for ws in wb.worksheets:
        # ★ 線の太さを統一：中太(medium)→細(thin)。
        #    原本テンプレは中太と細が約半々で混在しており、これが
        #    「太い線と細い線がランダムに見える」現象の正体。
        #    （二重線 double は様式上の意図とみなし維持する）
        for row in ws.iter_rows():
            for cell in row:
                b = cell.border
                sides = [getattr(b, s) for s in ("top", "bottom", "left", "right")]
                if any(sd and sd.style == "medium" for sd in sides):
                    def _cv(sd):
                        if sd and sd.style == "medium":
                            return Side(style="thin", color=sd.color)
                        return sd
                    cell.border = Border(
                        left=_cv(b.left), right=_cv(b.right), top=_cv(b.top), bottom=_cv(b.bottom),
