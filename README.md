...
## うまくログインできないとき（トラブルシュート）

**いちばん多い原因：** Cloud の Secrets 画面で `admin_password` の行が `[smtp]` より**下**にある。
TOMLでは `[smtp]` より下に書いた行はすべて「smtpの中の設定」になってしまいます。

```toml
# ✅ 正しい順番
admin_password = "あなたのパスワード"   # ← 必ず一番上（[smtp]より上）
...
修正したら **Save → アプリを Reboot** してください。

- ※ このアプリは v2 から、間違って `[smtp]` の中に書いてしまっても自動で見つけてログインできます（ただし正しい位置に直すのがおすすめ）。
- 緊急時は、アプリを動かすPC/サーバーの**環境変数 `ADMIN_PASSWORD`** でもログインできます（画面からは入れない安全な裏ルートです）。
- 原因が分からない時は、サイドバー最下部「🔧 デバッグ情報」を開くと、いま読めている設定キーの一覧（値は表示されません）が確認できます。
